...
int isAsciiDigit(int x) {
  int x1 = x + 1;   //本来应该是x + (~0x3X + 1),  这里为了节省符号先算了x + 1, 总体思路是0~8看作一类，9和10分别单独拿出来处理
  return !(((x1 + ~0x30)>>3) ^ 0) | !(x1 + ~0x38) | !(x1 + ~0x39) ;
} //这题还有一种思路是减去10是负的并且减去0是正的
...
int isLessOrEqual(int x, int y) {
  return (x>>31)&(!(y>>31)) | !((!(x>>31))&(y>>31)) & !((y+(~x+1))>>31);  //x<0,y>0符合跳出; x>0,y<0不符合跳出; 最后判断差
}
...
int logicalNeg(int x) {
   unsigned y = x;
   return ((~(~y+1)>>31)) & ((~y)>>31);   //这是自己想的方法，开始忽略了负数右移还是负数，后来才加了unsigned
    // return ((~(~x+1)&~x)>>31)&1;
...
unsigned floatPower2(int x) {
    if(x < -(126+23)) return 0; //小到无法用非规格数表示
    if(x > 127) return 0x7f800000;  
    if(x <= -127) return 1<<(149+x);   //用非规格数表示
    return (x+127)<<23;   //-126<=x<=127  用规格数表示
}

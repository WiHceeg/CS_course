...
private:
  ...
  static constexpr size_t ARP_REQUEST_DEFAULT_TTL = 5000;
  static constexpr size_t IP_MAC_PAIR_DEFAULT_TTL = 30000;

  std::unordered_map<uint32_t, std::pair<EthernetAddress, size_t>> arp_table_{};  // <ip地址, <Eth地址, 生存时间>>
  std::unordered_map<uint32_t, std::pair<std::list<InternetDatagram>, size_t>> waiting_arp_reply_dgram_{};  // <ip地址, <ip数据包, 生存时间>>
  std::queue<EthernetFrame> frame_out_{};
